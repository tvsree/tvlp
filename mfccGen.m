...
%-----------------------------------------------------------
  function mfccGen;
    ...
    Fs = 8000; nfft = 512;  % default values, updated from wavfile
...
% read the label file corresponding to current wav file for end points
...
% save LSF,RC, LAR, MFCC and FBenergy parameters into respective files, in KALDI format
                ...
                intSyLbl = str2num(wavFil(7:9));      % syllable label 1:150
...
%                  larFilID = fopen(['./QSexpt_Kaldi/' larFil],'w');
                ...
                mfcFil = wavFil ; mfcFil(end-2:end) = 'mfc';
                mfcFilID = fopen(['./QSexpt_Kaldi/' mfcFil],'w');
                fbeFil = wavFil ; fbeFil(end-2:end) = 'fbe';
                fbeFilID = fopen(['./QSexpt_Kaldi/' fbeFil],'w');
                frmCnt = size(ceps,2);
