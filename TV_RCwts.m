...
%----------------------------------------------------------------------------------------------
  function [Kpls Kmns Kavg RCpls RCmns RCavg resPls1 resMns1 resPls2 resMns2] = TV_RCwts(sig,lpcRdr,polRdr,type);
%----------------------------------------------------------------------------------------------
      ...
      for p = 1:lpcRdr,     % successive lattice stages
          ...
          Kpls(p,:) = PhiPls\(-PsyPls'); % set up matrix equations & solve for
          ...
          % compute pth stage output for TV_lattice and assign it as input to next stage iteration

          temp1 = resPls1 + [0 ; RCpls(p,1:sigL-1)'.* resMns1(1:sigL-1)];
          resMns1 = [0 ; resMns1(1:sigL-1)] + resPls1.*RCmns(p,:)';
          resPls1 = temp1;
          
% Alternatively, solve for Burg-avg formulation; same first stage input but different
...
% (Compute log Area-Ratio coefft contour, by a MSE fit to the stable regions
% of RCavg at each stage-p) Instead, adhoc correction below
