...
% The input signal file (may be re-sampled to Fs=8K; signal
% envelope is computed for recognizing speech utterance
...
%-----------------------------------------------------------
  function lsfVecGen;           % this version is to run on AULUS1 server
  
    global Fs sigL wndSiz wndShft;
    Fs = 8000; nfft = 512; filCnt = 0;
          
        spkrDirID = fopen(['/prosjekt/dBase_OLLO/spkrLst_chk'],'r');% spkr Directory list
...
% Autocorrelation analysis
                    ...
                    lsfAutContr(:,frmCnt) = lsfAut;  % saved as digital freq [0-pi]
...
% save LSF,RC & other parameter contour files, in a format suitable to KALDI
                ...
                lsfFil = [name '.lsf'];
                lsfFilID = fopen(['/prosjekt/tvs/QSchk_LSF/' lsfFil],'w');
                
                rcFil = [name '.rc'];
                rcFilID = fopen(['/prosjekt/tvs/QSchk_LSF/' rcFil],'w');
                
                larFil = [name '.lar'];
                larFilID = fopen(['/prosjekt/tvs/QSchk_LSF/' larFil],'w');
                ...
                for i = 1:frmCnt,
                    ...
                    fprintf(lsfFilID,'%9.0f',filCnt-1,i-1);
                    fprintf(lsfFilID,'%8.4f',lsfAutContr(:,i));
                    fprintf(lsfFilID,'%8.0f \n',PhnLbl);

                    fprintf(rcFilID,rcFil);
                    fprintf(rcFilID,'%9.0f',filCnt-1,i-1);
                    fprintf(rcFilID,'%8.3f',rcContr(:,i));
                    fprintf(rcFilID,'%8.0f \n',PhnLbl);

                    fprintf(larFilID,larFil);
                    fprintf(larFilID,'%9.0f',filCnt-1,i-1);
