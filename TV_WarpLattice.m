%------------------------------------------------------------
% Time-varying warped linear prediction (TV_WLP) analysis of
% speech using Lattice formulation and then extending it to 
...
% by Grenier, IEEE Tr ASSP, 1988, and extention by A. Harma
% et al., SPLett, May 2002.
% 
...
%----------------------------------------------------------------------------------------------
  function [WRCpls WRCmns WRCavg wresPls1 wresMns1 wresPls2 wresMns2] = TV_WRC(sig,lpcRdr,polRdr,basesType);
%----------------------------------------------------------------------------------------------

      sigL = length(sig);
      if ~isempty(strfind(basesType,'leg')),
          ...
          for i = 0:polRdr,     % polynomial bases upto polRdr
              if ~isempty(strfind(basesType,'pol')), 
                  bases(i+1,:) = ([0:sigL-1]/sigL).^i; % polynomial bases
              end
              if ~isempty(strfind(basesType,'sin')), 
...
% successive stage outputs, because of a single average K_ij solution, as below
          ...
          b_pm1
          for m = 1:sigL,   % all-pass filter residue signals instead of just delay
...
%---------------------------------------------------------------
%-------------------------------------------------------
% function implementing first order all-pass filter
% whose output has warped frequency property of the 
% input signal. 0 < lamda < 1 is the warping parameter.
%-------------------------------------------------------
  function wrpsig = warpFilt(sig,lamda);
%----------------------------------------
    sigL = length(sig);
